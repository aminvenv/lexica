...
SEED_NUM = 42
tokenizer = BertTokenizer.from_pretrained(TOKENIZER, cache_dir='./cache')
...
class MsmarcoDocumentQueryPair(Dataset):
    def __init__(self, path):
        self.tokenizer = BertTokenizer.from_pretrained(TOKENIZER, cache_dir='./cache')
        self.path = path
        self.queries = []
        self.passages = []
...
def collate_fn(batch):
    ...
    passage_inputs = tokenizer(passages,
                               return_tensors="pt",
                               padding=True,
                               max_length=512,
                               truncation=True)
    ...
    query_inputs = tokenizer(queries,
                             return_tensors="pt",
                             padding=True,
                             max_length=512,
                             truncation=True)
...
def main(args):
    ...
    for i in range(1, MAX_EPOCHS+1):
        ...
        model_to_save = TILDE.load_from_checkpoint(model_type=MODEL_TYPE, checkpoint_path=os.path.join(args.save_path, 'epoch_'+str(i)+'.ckpt'))
        model_to_save.save(os.path.join(args.save_path, 'TILDE_EPOCH'+str(i)))
